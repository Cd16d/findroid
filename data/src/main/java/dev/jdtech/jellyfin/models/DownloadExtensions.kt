...
fun formatSeasonRanges(
    ...
    if (
        context != null &&
            seasonSingleRes != null &&
            seasonPairRes != null &&
            seasonMultipleRes != null
    ) {
...
fun formatSeasonsString(
    ...
    val epString =
        if (context != null && episodesPluralRes != null) {
...
fun FindroidItem.getQualityLabel(): String {
    val videoStream =
        sources.flatMap { it.mediaStreams }.firstOrNull { it.type == MediaStreamType.VIDEO }
    val height = videoStream?.height
    val isHdr =
        videoStream?.videoRangeType != null && videoStream.videoRangeType != VideoRangeType.SDR
    return when {
