...
fun DownloadsScreen(
    ...
    showToDelete?.let { showItem ->
        ...
        ConfirmDeleteDialog(
            title = stringResource(CoreR.string.delete_download),
            message =
                stringResource(CoreR.string.delete_item_confirm, showItem.show.name, sizeFormatted),
            onConfirm = {
    ...
    if (showBatchDeleteDialog) {
        ...
        val totalBytes =
            selectedMovies.sumOf { it.diskSize() } + selectedShows.sumOf { it.totalDiskSize }
    ...
    val allMovies =
        remember(state.movies, state.activeDownloads) {
    ...
    val allShows =
        remember(state.shows, state.activeDownloads) {
            val dbShowIds = state.shows.map { it.show.id }.toSet()
            val queuedEpisodeShows =
                state.activeDownloads
                    .asSequence()
                    .map { it.item }
    ...
    Scaffold(
            ...
            if (allMovies.isNotEmpty() || allShows.isNotEmpty() || state.isSelectionMode) {
                TopAppBar(
                    ...
                    navigationIcon = {
                        if (state.isSelectionMode) {
                            IconButton(
                                onClick = { viewModel.onAction(DownloadsAction.ExitSelectionMode) }
                            ) {
                                Icon(
                    ...
                    actions = {
                        if (state.isSelectionMode) {
                            ...
                            val isAllSelected =
                                totalItemCount > 0 && state.selectedItemIds.size >= totalItemCount
                            TextButton(
        ...
        Box(modifier = Modifier.fillMaxSize().padding(top = innerPadding.calculateTopPadding())) {
                ...
                LazyColumn(
                    ...
                    if (allMovies.isNotEmpty()) {
                        item(key = "movies_section_header") {
                            ...
                            val moviesSizeFormatted =
                                Formatter.formatFileSize(context, totalMoviesBytes)
                            DownloadSectionHeader(
                                title = stringResource(CoreR.string.movies_section_title),
                                itemCount = allMovies.size,
                                totalSizeFormatted =
                                    if (totalMoviesBytes > 0) moviesSizeFormatted else "",
                            )
                        }
                    }
                    ...
                    items(
                        ...
                        val isDownloading =
                            activeEntry?.state is DownloadQueue.EntryState.Downloading ||
                                (isMoviePaused &&
                                    (activeEntry.bytesDownloaded > 0 ||
                                        activeEntry.downloadId != null))
                        val isPending =
                            activeEntry?.state is DownloadQueue.EntryState.Pending ||
                                (isMoviePaused &&
                                    activeEntry.bytesDownloaded <= 0 &&
                                    activeEntry.downloadId == null)
                        ...
                        val dlProgress =
                            when {
                                ...
                                activeEntry != null ->
                                    (activeEntry.progress / 100f).coerceIn(0f, 1f)
                                else -> 0f
                            }
                        val etaFormatted =
                            if (
                                activeEntry != null &&
                                    activeEntry.bytesPerSecond > 0 &&
                                    activeEntry.totalBytes > activeEntry.bytesDownloaded
                            ) {
                                val remainingSec =
                                    (activeEntry.totalBytes - activeEntry.bytesDownloaded) /
                                        activeEntry.bytesPerSecond
                        ...
                        when (status) {
                            DownloadStatus.TRANSFERRING -> {
                                ...
                                metadataText =
                                    "$transferPercent% • ${stringResource(CoreR.string.moving_storage_short)}"
                                playbackProgress = 0f
                            }
                            DownloadStatus.CONVERTING -> {
                                val convProgressStr =
                                    if (dlProgress > 0f) "${(dlProgress * 100).toInt()}% • " else ""
                                metadataText =
                                    "$convProgressStr${stringResource(CoreR.string.converting)}"
                                playbackProgress = 0f
                            }
                            DownloadStatus.DOWNLOADING -> {
                                metadataText =
                                    when {
                                        isMoviePaused ->
                                            "${(dlProgress * 100).toInt()}% • ${stringResource(CoreR.string.paused)}"
                                        etaFormatted.isNotEmpty() ->
                                            "${(dlProgress * 100).toInt()}% • $etaFormatted"
                            ...
                            DownloadStatus.DOWNLOADED -> {
                                ...
                                metadataText =
                                    when {
                                        yearText.isNotEmpty() && durationText.isNotEmpty() ->
                                ...
                                playbackProgress =
                                    if (movie.runtimeTicks > 0) {
                                        (movie.playbackPositionTicks.toFloat() / movie.runtimeTicks)
                                            .coerceIn(0f, 1f)
                                    } else 0f
                            }
                        }
                        ...
                        DownloadItemCard(
                            ...
                            state =
                                DownloadItemCardState(
                                    ...
                                    isPaused =
                                        activeEntry?.state is DownloadQueue.EntryState.Paused,
                            ...
                            actions =
                                DownloadCardActions(
                                    onClick = {
                                        if (state.isSelectionMode) {
                                            viewModel.onAction(
                                                DownloadsAction.ToggleSelection(movie.id)
                                            )
                    ...
                    if (allShows.isNotEmpty()) {
                        item(key = "shows_section_header") {
                            ...
                            val showsSizeFormatted =
                                Formatter.formatFileSize(context, totalShowsBytes)
                            DownloadSectionHeader(
                                title = stringResource(CoreR.string.shows_section_title),
                                itemCount = allShows.size,
                                totalSizeFormatted =
                                    if (totalShowsBytes > 0) showsSizeFormatted else "",
                            )
                        }
                    }
                    ...
                    items(
                        ...
                        val downloadingEp = activeEpisodes.firstOrNull {
                            it.state is DownloadQueue.EntryState.Downloading ||
                                (isAllPaused && it.bytesDownloaded > 0)
                        }
                        ...
                        val failedEps = activeEpisodes.filter {
                            it.state is DownloadQueue.EntryState.Failed
                        }

                        val showTransfer =
                            state.activeTransfers[showItem.show.id]
                                ?: showItem.episodes.firstNotNullOfOrNull {
                                    state.activeTransfers[it.id]
                                }
                        ...
                        val showQueue =
                            state.allQueueEntries.filter {
                                it.item is FindroidEpisode &&
                                    (it.item as FindroidEpisode).seriesId == showItem.show.id
                            }
                        val completedInQueue = showQueue.count {
                            it.state is DownloadQueue.EntryState.Completed
                        }
                        val activeProgress =
                            if (effectiveEp != null) (effectiveEp.progress / 100f).coerceIn(0f, 1f)
                            else 0f
                        val totalProgress =
                            if (showQueue.isNotEmpty()) {
                                ((completedInQueue.toFloat() + activeProgress) /
                                        showQueue.size.toFloat())
                                    .coerceIn(0f, 1f)
                            } else {
                                activeProgress
                            }
                        ...
                        when (status) {
                            ...
                            DownloadStatus.CONVERTING -> {
                                ...
                                val epLabel =
                                    if (ep != null && ep.indexNumber > 0) "E${ep.indexNumber}"
                                    else ep?.name ?: ""
                                val convProgressStr =
                                    if (totalProgress > 0f) "${(totalProgress * 100).toInt()}% • "
                                    else ""
                                metadataText =
                                    if (epLabel.isNotEmpty())
                                        "$epLabel • $convProgressStr${stringResource(CoreR.string.converting)}"
                                    else
                                        "$convProgressStr${stringResource(CoreR.string.converting)}"
                            }
                            DownloadStatus.DOWNLOADING -> {
                                ...
                                val isEpPaused =
                                    downloadingEp?.state is DownloadQueue.EntryState.Paused ||
                                        isAllPaused
                                val epEtaFormatted =
                                    downloadingEp?.etaSeconds?.let { formatStableEta(it) } ?: ""
                                val totalProgressInt = (totalProgress * 100).toInt()
                                metadataText =
                                    when {
                                        isEpPaused ->
                                            if (epLabel.isNotEmpty())
                                                "$epLabel • $totalProgressInt% • ${stringResource(CoreR.string.paused)}"
                                            else stringResource(CoreR.string.paused)
                                        epEtaFormatted.isNotEmpty() ->
                                            "$epLabel • $totalProgressInt% • $epEtaFormatted"
                                        else ->
                                            if (epLabel.isNotEmpty())
                                                "$epLabel • $totalProgressInt%"
                                            else "$totalProgressInt%"
                                    }
                            }
                            DownloadStatus.PENDING -> {
                                metadataText =
                            ...
                            DownloadStatus.DOWNLOADED -> {
                                ...
                                metadataText =
                                    when {
                                        yearText.isNotEmpty() &&
                                            showItem.seasonsFormatted.isNotEmpty() ->
                            ...
                            state =
                                DownloadItemCardState(
                                    ...
                                    pendingDeletionSeconds =
                                        state.pendingDeletionIds[showItem.show.id],
                                ),
                            actions =
                                DownloadCardActions(
                                    onClick = {
                                        if (state.isSelectionMode) {
                                            viewModel.onAction(
                                                DownloadsAction.ToggleSelection(showItem.show.id)
                                            )
                                        } else {
                                    ...
                                    onSwipeDelete = {
                                        viewModel.onAction(
                                            DownloadsAction.StageDeleteShow(showItem)
                                        )
                                    },
                                    onPauseDownload = {
                                        viewModel.onAction(
                                            DownloadsAction.PauseDownload(showItem.show.id)
                                        )
                                    },
                                    onResumeDownload = {
                                        viewModel.onAction(
                                            DownloadsAction.ResumeDownload(showItem.show.id)
                                        )
                                    },
                                    onUndoDelete = {
                                        viewModel.onAction(
                                            DownloadsAction.UndoDelete(showItem.show.id)
                                        )
                                    },
                                ),
                        )
            ...
            AnimatedVisibility(
                ...
                val selectedActive =
                    state.activeDownloads.filter { entry ->
                        state.selectedItemIds.contains(entry.id) ||
                            (entry.item is FindroidEpisode &&
                                state.selectedItemIds.contains(
                                    (entry.item as FindroidEpisode).seriesId
                                ))
                    }
