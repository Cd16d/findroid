...
/**
 * A floating, compact Material 3 capsule selection toolbar. Floats above the content instead of
        ...
        Surface(
            ...
            Row(
                ...
                if (anyDownloadingOrPending != null && onPauseOrResume != null) {
                    IconButton(onClick = { onPauseOrResume(anyDownloadingOrPending) }) {
                        Icon(
                            painter =
                                painterResource(
                                    if (anyDownloadingOrPending) CoreR.drawable.ic_pause
                                    else CoreR.drawable.ic_play
                                ),
                            contentDescription =
                                stringResource(
                                    if (anyDownloadingOrPending) CoreR.string.pause_all
                                    else CoreR.string.resume_all
                                ),
