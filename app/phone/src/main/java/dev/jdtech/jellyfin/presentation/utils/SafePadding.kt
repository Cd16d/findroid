...
fun rememberSafePadding(
    ...
    val safePaddingStart =
        if (handleStartInsets) {
            ...
            // Navigation rail handles safe drawing inset in medium and expanded width, or compact
            // height (phone landscape)
            when {
                ...
                else -> with(density) { safeInsets.getLeft(this, layoutDirection).toDp() }
            }
        }

    val safePaddingTop = with(density) { safeInsets.getTop(this).toDp() }
    val safePaddingEnd = with(density) { safeInsets.getRight(this, layoutDirection).toDp() }
    val safePaddingBottom =
        if (handleBottomInsets) {
